...
function Broadcast(el, imagesAndUrl, JSON) {
  ...
  this.intervalTime = JSON.intervalTime || 5000; // 定义一些全局变量
...
Broadcast.prototype.init = function () {
  // 动态添加一些css样式
  var cssStr = ".broadcastMe .broadcastMe-list {width: ".concat((this.imagesAndUrl.length + 2) * this.el.clientWidth, "px;}.broadcastMe .broadcastMe-list .broadcastMe-item {width:").concat(this.el.clientWidth, "px;}");
  var styleNode = document.createElement('style');
  styleNode.innerText = cssStr;
  document.head.appendChild(styleNode);
  var html = "<div class=\"broadcastMe\">\n                <div class=\"broadcastMe-list\" style=\"left:-".concat(this.el.clientWidth, "px\">"); // 添加显示图片区域
  // 无缝轮播，收尾多加一张

  var temStr = "<div class=\"broadcastMe-item\">\n          <a href=\"".concat(this.imagesAndUrl[this.imagesAndUrl.length - 1].linkHref == null ? "#" : this.imagesAndUrl[this.imagesAndUrl.length - 1].linkHref, "\"><img src=\"").concat(this.imagesAndUrl[this.imagesAndUrl.length - 1].imgSrc, "\" alt=\"\u8F6E\u64AD\u56FE\u56FE\u7247-pawn\"></a>\n        </div>");
  this.imagesAndUrl.map(function (item) {
    temStr += "<div class=\"broadcastMe-item\">\n                  <a href=\"".concat(item.linkHref == null ? "#" : item.linkHref, "\"><img src=\"").concat(item.imgSrc, "\" alt=\"\u8F6E\u64AD\u56FE\u56FE\u7247-pawn\"></a>\n              </div>");
  });
  temStr += "<div class=\"broadcastMe-item\">\n              <a href=\"".concat(this.imagesAndUrl[0].linkHref == null ? "#" : this.imagesAndUrl[0].linkHref, "\"><img src=\"").concat(this.imagesAndUrl[0].imgSrc, "\" alt=\"\u8F6E\u64AD\u56FE\u56FE\u7247-pawn\"></a>\n            </div>");
  html += temStr + "</div>";
  temStr = "<div class=\"broadcastMe-tool\">\n              <div class=\"broadcastMe-spot broadcastMe-spot-active\"></div>"; // 添加下面小圆点

  for (var i = 1, len = this.imagesAndUrl.length; i < len; i++) {
    temStr += "<div class=\"broadcastMe-spot\"></div>";
  }

  html += temStr + "</div>"; // 添加左右2边按钮

  temStr = "<div class=\"broadcastMe-btn broadcastMe-btn-left\"><</div>\n            <div class=\"broadcastMe-btn broadcastMe-btn-right\">></div>";
  html += temStr;
  this.el.innerHTML += html + "</div>"; // 调用绑定事件
...
Broadcast.prototype.bindEvent = function () {
  ...
  this.broadcastMeBtnRight = this.el.getElementsByClassName('broadcastMe-btn-right')[0]; // 左右按钮 事件监听
  ...
  this.el.addEventListener("touchend", function () {
    var left = _this.broadcastMeList.offsetLeft; // 判断正在滚动的图片距离左右图片的远近，

    _this.index = Math.round(-left / _this.el.clientWidth);
    _this.animationMark = false; // 开启定时器
...
Broadcast.prototype.render = function () {
  ...
  if (this.animationMark) return;
  this.animationMark = true; // 修改broadcastMeList 的left值
